...
window.throttle = function (callback, interval) {
  let enableCall = true
...
axios.get("/api/data").then((response) => {
  ...
  swiper.on("lazyImageReady", () => {
    document.querySelectorAll(".swiper-slide > div").forEach((i) => {
      i.classList.remove(...["invisible", "opacity-0", "transition-none"])
  ...
  function RadarChart(id, data, options) {
    var cfg = {
      ...
      strokeWidth: 7, //The width of the stroke around each blob
  ...
  const skills = response.data.radarChart.skills

  var data = [[]]

  let roundStrokes = JSON.parse(
  ...
  Object.keys(skills).forEach((skill) => {
    data[0].push({
      axis: skill,
      value: roundStrokes ? skills[skill] * 0.95 : skills[skill],
    })
  })
  ...
  grad
    .selectAll("stop")
    .data(response.data.radarChart.colors)
    .enter()
    .append("stop")
    ...
    .attr("offset", function (d, i) {
      return 100 * (i / (response.data.radarChart.colors.length - 1)) + "%"
    })
...
window.blinkText = function (e) {
  ...
  section.classList.add("gradient", "text-gradient", "duration-75")
  setTimeout(function () {
    section.classList.remove("gradient", "text-gradient")
  }, 1000)
}
...
window.addEventListener("scroll", throttle(updateUrl, 500))
