<header x-data="{ mobileMenuOpen: false }" class="fixed w-full z-30">
    ...
    <div class="z-40 px-12 mx-auto xl:px-8 backdrop-blur-sm">
        <div class="flex items-center justify-between h-16 md:justify-start md:space-x-6">
            <div class="inline-flex">
                <a href="#hero" class="flex items-center justify-center space-x-3 transition-all duration-1000 ease-out transform">
                    @if(settingImage('logo'))
                        {{-- @if($jd)
                            <h1 class="text-xl font-extrabold tracking-widest">
                                <span class="text-transparent bg-clip-text bg-gradient-to-r from-pink-600 via-fuchsia-600 to-purple-600">jd&#62;</span><span class="animate-blink text-purple-600">_</span>
                            </h1>
                        @endif --}}

                        @if($ivno)
                            <img class="w-8 h-8 object-scale-down @if(Portfolio::setting('ivno.logo_rotation')) animate-rotation @endif" src="{{ settingImage('logo') }}" alt="ivno.logo">
                        @endif
            ...
            <div class="flex justify-end flex-grow -my-2 -mr-2 md:hidden">
                ...
                @include('app.menus.guest')
        </div>
    </div>

        @include('app.menus.guest-mobile')
        
        <div class="absolute overlay -z-10 inset-0 w-full h-full bg-accent"></div>
</header>
