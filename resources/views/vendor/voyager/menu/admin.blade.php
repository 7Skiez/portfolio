...
@foreach ($items as $item)
    ...
    <li class="dd-item" data-id="{{ $item->id }}">
        <div class="pull-right item_actions" style="display: flex; align-items: center">
            @if (Auth::user()->hasRole('admin'))    
                <div class="btn btn-sm btn-danger pull-right delete" style="margin-right:5px" data-id="{{ $item->id }}">
                    <i class="voyager-trash"></i> {{ __('voyager::generic.delete') }}
                </div>
            @endif

            <input type="checkbox" class="toggleswitch featured" data-id="{{ $item->id }}" @if($item->featured) checked @endif>
