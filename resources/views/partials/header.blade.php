            ...
            <div class="inline-flex">
                <a href="#hero" class="flex items-center justify-center space-x-3 transition-all duration-1000 ease-out transform">
                    @if(image($portfolio->logo))
                        ...
                        @if($ivno)
                            <img class="w-8 h-8 object-scale-down @if(setting('ivno.logo_rotation')) animate-rotation @endif" src="{{ image(setting('ivno.logo')) }}" alt="ivno.logo">
                        @endif
                    @else
