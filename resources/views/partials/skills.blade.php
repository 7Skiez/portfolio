<section id="{{ $portfolioSections[$i]['id'] }}" class="relative w-full pt-24 xl:pt-36 xl:pb-4">
    ...
    <div class="relative flex items-center justify-center w-full">
        ...
        @if ($ivno)
            <div class="relative flex flex-col w-full items-center justify-center">
                ...
                <h3 class="text-sm xl:text-xl mb-12 font-bold leading-none text-white text-opacity-30 text-center">Other {{ $portfolioSections[$i]['title'] }}</h3>
                <div class="flex flex-col items-center space-y-4">
                    @php
                        preg_match_all('/(?<=--).*?(?=--)/', setting('ivno.other_skills'), $otherSkills);
                    @endphp
