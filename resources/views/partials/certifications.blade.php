    ...
    <div class="relative flex items-center justify-center w-full">
        <div class="relative flex flex-col xl:flex-row justify-center items-center">
            @foreach ($portfolio->certifications as $cert)
                <a href="{{ $cert->link }}" target="_blank" class="flex flex-col p-4 w-52 h-52 xl:w-60 xl:h-60 mx-4 my-4 rounded-xl justify-center items-center hover:scale-110 duration-300" style="background: linear-gradient({{ $cert->bg_rotation }}deg, {{ setting('ivno.radar_color_one') }} 0%, {{ setting('ivno.radar_color_two') }} 100%)">
                    ...
                    <span class="w-4/5 text-center color-[#0C041B] font-[georgia] font-semibold tracking-wider text-base">{{ $cert->name }}</span>
                </a>
            @endforeach
        </div>
    </div>
