...
@section('content')
    @foreach ($section->items as $cert)
        <a href="{{ $cert->link }}" target="_blank" class="flex flex-col p-4 w-52 h-52 xl:w-60 xl:h-60 mx-4 my-4 rounded-3xl xl:rounded-xl justify-center items-center hover:scale-110 duration-300" style="background:{{ preg_replace('/(?<=\()([^[+-]?([0-9]+\.?[0-9]*|\.[0-9]+))(?=deg)|(?<=\()([0-9]+)(?=deg)/',$cert->bg_rotation,Portfolio::setting('ivno.main_gradient')) }}">
